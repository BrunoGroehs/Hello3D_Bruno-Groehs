...
bool loadTrajectory(const string &filePath, vector<glm::vec3> &points)
{
    ...
    arq.close();
    cout << "Trajetoria carregada: " << points.size() << " pontos" << endl;
    return true;
}
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (trajectoryPoints.size() >= 4)
        {
            ...
            if (trajectoryActive)
            {
                ...
                while (trajectoryT >= (float)n) trajectoryT -= (float)n;
            }
        ...
        for (int i = 0; i < (int)sceneObjects.size(); i++)
        {
            ...
            if (i == selectedObject) pos = pos + trajPos;
